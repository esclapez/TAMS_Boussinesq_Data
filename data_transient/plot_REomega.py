"""Plot number of work normalised RE from TAMS and DNS."""
...
if __name__ == "__main__":
    ...
    for i in range(len(dataset)):
        ...
        cost_map = np.load(f"./{dataset[i]}/cost_map{suffix}.npy")
        ext_map = np.load(f"./{dataset[i]}/ext_map{suffix}.npy")
        work_norm_RE = pRE_map * np.sqrt(cost_map * nruns)
        MC_equil = (
            np.sqrt((1 - pmean_map) / (pmean_map * pRE_map**2) * base_cost_traj)
            * pRE_map
        )
        plt.scatter(pmean_map, work_norm_RE, color=colors[i], label=labels[i], s=15)

        if i == len(dataset) - 1:
            plt.scatter(pmean_map, MC_equil, color="r", label="DNS", s=10)
        else:
            plt.scatter(pmean_map, MC_equil, color="r", s=10)

    plt.xlim(left=1e-8, right=1.0)
    plt.ylim(bottom=3e1, top=1e6)
    plt.gcf().axes[0].set_xscale("log", base=10)
    plt.gcf().axes[0].set_yscale("log", base=10)
    plt.hlines(1.0, 1e-8, 1.0, color="k", linestyle="--")
    plt.legend(fontsize=fsize, loc=3)

    plt.xlabel(r"$\overline{P}_K$", fontsize=fsize)
    plt.ylabel(r"$\mathrm{RE}_{\omega}$", fontsize=fsize)
    plt.xticks(fontsize=fsize)
    plt.yticks(fontsize=fsize)
    plt.tight_layout()
    plt.show()
    # plt.savefig("REomega_withDNS_insert.pdf")
